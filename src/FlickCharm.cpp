...
struct FlickData {
    ...
    if (lastPosValid) {
      ...
      if (timeElapsed) {
        ...
        if (state == AutoScrollAcceleration) {
          ...
          if ((oldSpeedY <= 0 && newSpeedY <= 0) || (oldSpeedY >= 0 && newSpeedY >= 0 && oldSpeedX <= 0 && newSpeedX <= 0)
  ...
  bool scrollWidget(const int dx, const int dy)
  {
    ...
    if (scrollArea) {
      ...
      return (scrollArea->horizontalScrollBar()->value() != x || scrollArea->verticalScrollBar()->value() != y);
    }
...
bool FlickCharm::eventFilter(QObject * object, QEvent * event)
{
  ...
  if (type == QEvent::MouseMove && mouseEvent->buttons() != Qt::MiddleButton)
    return false;
